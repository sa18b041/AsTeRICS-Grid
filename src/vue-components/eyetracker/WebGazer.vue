...
export default {
  ...
  async created() {
    ...
    if (window && !this.off) {
      ...
      await webgazer
        .setRegression("ridge")
        .setGazeListener(function(data) {  // it is an given event - where a callback function is called : here the data will be übergeben - where the user looks x/y values
          if (data) {
            thiz.x = data.x;
            thiz.y = data.y;
             let dataCoord = {
