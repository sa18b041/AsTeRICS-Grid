<template>
  <div class="box" v-cloak>
    <header
      class="row header"
      role="banner"
      v-if="metadata && !metadata.fullscreen"
    >
      ...
      <div class="btn-group left">
        ...
        >
          ...
          <span class="hide-mobile" data-i18n
            >Editing on // Bearbeiten ein</span
          >
        </button>
        <button
          tabindex="31"
        ...
        >
          ...
          <span class="hide-mobile" data-i18n
            >Input options // Eingabeoptionen</span
          >
        </button>
      </div>
      ...
      <button tabindex="32" @click="applyFullscreen()" class="spaced small">
    ...
    <div
      ...
      v-show="
        viewInitialized &&
          ...
          (!globalGridData || globalGridData.length === 0)
      "
    >
      <div data-i18n="" style="margin-top: 2em">
        <span
          >No elements, click <a :href="'#grid/edit/' + gridId">Edit grid</a> to
          enter edit mode.</span
        >
        <span
          >Keine Elemente, klicke auf
          <a :href="'#grid/edit/' + gridId">Grid bearbeiten</a> um das Grid zu
          bearbeiten.</span
        >
      </div>
    </div>
...
import{ Webgazer} from "../../js/input/eyeTrackerInput";
...
let vueConfig = {
 ...
 onUpdate(coord){
    ...
    initInputMethods() {
      ...
      // if (inputconfig.eyeTrackerInput) {
      //   thiz.webgazer = Webgazer.getInstanceFromConfig(
      //     inputConfig,
      //     "webgazerActive"
      //   );
      //   thiz.webgazer.setSelectionListener(setSelectionListener);
      //   gridInstance.setLayoutChangedStartListener(function() {
      //     thiz.webgazer.pauseEyetracking();
      //   });
      //   gridInstance.setLayoutChangedEndListener(function() {
      //     thiz.Webgazer.resumeEyetracking();
      //   });

      //   // thiz.webgazer.startEyetracking();
      //   thiz.webgazer.onUpdate();
      // }
    ...
    reloadFn(event, updatedIds, updatedDocs) {
      ...
      let updatedGridDoc = updatedDocs.filter(
        (doc) => vueApp.gridData && doc.id === vueApp.gridData.id
      )[0];
      ...
      if (
        this.updatedMetadataDoc &&
        this.updatedMetadataDoc.lastOpenedGridId !== vueApp.gridData.id
      ) {
        dataService
    ...
    dataService
      ...
      .then((gridData) => {
        ...
        if (gridData.hasAREModel()) {
          ...
          areService.uploadAndStartModel(
            areModel.dataBase64,
            gridData.getAREURL(),
            areModel.fileName
          );
      ...
      .then((savedMetadata) => {
        let metadata = JSON.parse(
          JSON.stringify(savedMetadata || new MetaData())
        );
        metadata.lastOpenedGridId = this.gridId;
        metadata.locked =
          metadata.locked === undefined
            ? urlParamService.isDemoMode() &&
              dataService.getCurrentUser() === constants.LOCAL_DEMO_USERNAME
            : metadata.locked;
        metadata.fullscreen =
...
function stopInputMethods() {
  ...
  if (vueApp)
    $(document).off(constants.EVENT_GRID_RESIZE, vueApp.resizeListener);
  ...
  if (vueApp && vueApp.webgazer) vueApp.webgazer.destroy();
...
function initContextmenu() {
  ...
  let contextItems = {
    ...
    CONTEXT_EYETRACKER: {
      name: getName(
        "Eyetracking input",
        "Augensteuerung Eingabe",
        inputConfig.eyetrackingEnabled
      ),
      icon: "fas fa-eye",
